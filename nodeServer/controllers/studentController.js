...
LEFT JOIN parents p1 ON s.parent1Id = p1.parentId
LEFT JOIN parents p2 ON s.parent2Id = p2.parentId
LEFT JOIN contact ct ON s.contactId = ct.contactId
...
exports.editStudent = async (req, res) => {
    const {
        firstName, lastName, idNumber, birthDate, siblingsSum, city, streetName, buildingAndApartmentNumber, image,
        fatherName, fatherCity, fatherStreet, fatherBuildingAndApt, fatherPhone, fatherFamilyStatus, fatherOccupation,
        motherName, motherCity, motherStreet, motherBuildingAndApt, motherPhone, motherFamilyStatus, motherOccupation,
        contactName, contactPhone, contactRelation, contactCity, contactStreet, contactBuildingAndApt,
        profileId, welfareEligibilityStatusId, welfareEligibilityStatusDate,
        paymentStatusId, wasPaid, discount,addressId,
        fatherAddressId,
        motherAdrdessId,
        contactAddressId,
        userId,
    ...
    const hebrewMonths = ['ניסן','אייר','סיוון','תמוז','אב','אלול','תשרי','חשוון','כסלו','טבת','שבט','אדר'];//החודשים מתחילים מניסן כי ככה הספריה בנויה
    const hebrewDates = ['א','ב','ג','ד','ה','ו','ז','ח','ט','י','יא', 'יב', 'יג', 'יד', 'טו', 'טז', 'יז', 'יח', 'יט', 'כ', 'כא', 'כב', 'כג', 'כד', 'כה', 'כו', 'כז', 'כח', 'כט', 'ל'];
    ...
    if (birthDate) {
        ...
        SET streetName = @streetName, buildingAndApartmentNumber = @buildingAndApartmentNumber, cityId = @city
        WHERE addressId = @addressId;
            `);
            if (!addressResult || addressResult.recordset.length === 0) throw new Error('Failed to update address');
        }
        ...
        if (fatherAddressId){
            ...
            .input('fatherCity', sql.Int, fatherCity || null)
            .query(`UPDATE address
               SET  streetName = @fatherStreet, buildingAndApartmentNumber = @fatherBuildingAndApt, cityId = @fatherCity
                WHERE addressId = @fatherAddressId;
            `);
            if (!fatherAddressResult || fatherAddressResult.recordset.length === 0) throw new Error('Failed to insert dad address');
        ...
        if (motherAddressId){
            ...
            .input('motherCity', sql.Int, motherCity || null)
            .query(`UPDATE address
                SET streetName = @motherStreet, buildingAndApartmentNumber = @motherBuildingAndApt, cityId = @motherCity
                WHERE addressId = @motherAddressId;
            `);
            if (!motherAddressResult || motherAddressResult.recordset.length === 0) throw new Error('Failed to insert mom address');
        }
        ...
        if(contactAddressId){
            ...
            .input('contactCity', sql.Int, contactCity || null)
            .query(`UPDATE address
                SET streetName = @contactStreet, buildingAndApartmentNumber = @contactBuildingAndApt, cityId = @contactCity)
        ...
        if(userId){
            ...
            .input('birthDate', sql.Date, birthDate || null)
            .input('hebrewBirthMonth', sql.NVarChar, hebrewBirthMonth || null)
            .input('hebrewDate', sql.Int, hebrewDate || null)
            .input('addressId', sql.Int, addressId || null)
            .query(`UPDATE [user]
                firstName = @firstName, lastName = @lastName, idNumber = @idNumber, birthDate = @birthDate, addressId = @addressId, hebrewDateMonth = @hebrewBirthMonth, hebrewDate = @hebrewDate
                WHERE userId = @userId;
            `);
            if (!UserResult || UserResult.recordset.length === 0) throw new Error('Failed to insert user');
        }
        ...
        if(contactId){
            ...
            SET contactName = @contactName, contactPhone = @contactPhone, contactRelation = @contactRelation, addressId = @contactAddressId
        ...
        const studentResult = await request.input('userId', sql.Int, userId || null)
            ...
            .query(`UPDATE student
                SET userId = @userId, siblingsSum = @siblingsSum, welfareEligibilityStatusId = @welfareEligibilityStatusId, welfareEligibilityStatusDate = @welfareEligibilityStatusDate, profileId = @profileId, imageId = @imageId
        ...
        if(studiesPaymentId){
            ...
            .input('wasPaid', sql.Bit, wasPaid || null)
            .input('discount', sql.Bit, discount || null)
            .query(`UPDATE studiesPayment
            SET studentId = @studentId, paymentStatusId = @paymentStatusId, wasPaid = @wasPaid, discount = @discount
           WHERE studiesPaymentId = @studiesPaymentId;
            `);
            if (!paymentResult || paymentResult.recordset.length === 0) throw new Error('Failed to insert studiesPayment')
        }
        else{
        ...
        const fatherResult = await fatherRequest.input('studentId', sql.Int, studentId || null)
            ...
            .query(`UPDATE parents 
                SET studentId = @studentId, parentName = @fatherName,addressId = @fatherAddressId,phoneNumber = @fatherPhone,job = @fatherOccupation,familyStatusId = @fatherFamilyStatus, gender = 0
        ...
        const motherResult = await motherRequest.input('studentId', sql.Int, studentId || null)
            ...
            .input('motherFamilyStatus', sql.Int, motherFamilyStatus || null)
            .query(`UPDATE parents 
                SET studentId = @studentId, parentName = @motherName,addressId = @motherAddressId,phoneNumber = @motherPhone,job = @motherOccupation,familyStatusId = @motherFamilyStatus,gender = 1
                WHERE parentId = parentId;`)
        if (!motherResult || motherResult.recordset[0].length === 0) throw new Error('Failed to insert mother');
        const motherId = motherResult.recordset[0].parentId;
